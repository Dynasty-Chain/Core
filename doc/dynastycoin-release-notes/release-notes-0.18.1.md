DynastyCoin Core version 0.18.1 is now available from:

  <https://download.dynastycoin.org/dynastycoin-0.18.1/>
...
`/Applications/DynastyCoin-Qt` (on Mac) or `dynastycoind`/`dynastycoin-qt` (on
Linux).
...
DynastyCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use DynastyCoin Core on unsupported systems.
...
10.10. Additionally, DynastyCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:22955:22955` (this is an extra `:22955` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@dynastycoin.org) the DynastyCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 DNST),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in DNST per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/dynastycoin-conf.md)
  about the `dynastycoin.conf` file describes how to use it to configure
  DynastyCoin Core.

- A new document introduces DynastyCoin Core's BIP174 [Partially-Signed
  DynastyCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent DynastyCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  DynastyCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when DynastyCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of DynastyCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart dynastycoind with the `-deprecatedrpc=generate`
...
- A new `dynastycoin-wallet` tool is now distributed alongside DynastyCoin
...
This section describes planned changes to DynastyCoin Core that may affect
other DynastyCoin software and services.

- Since version 0.16.0, DynastyCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with DynastyCoin Core 0.20 (expected about
  a year after 0.18), DynastyCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the DynastyCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in DynastyCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any DynastyCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, DynastyCoin Core
...
- A sub-project of DynastyCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with DynastyCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to DynastyCoin Core's own implementation, although entropy
  gathered by DynastyCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves DynastyCoin Core a
...
- On macOS, DynastyCoin Core now opts out of application CPU throttling
...
- #22955 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
