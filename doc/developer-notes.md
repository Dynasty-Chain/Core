...
- [Development guidelines](#development-guidelines)
    - [General DynastyCoin Core](#general-dynastycoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
DynastyCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play dynastycoins" on the test network, if you
...
DynastyCoin Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_dynastycoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_dynastycoin --log_level=test_suite
$ valgrind -v --leak-check=full src/dynastycoind -printtoconsole
...
# A coverage report will now be accessible at `./test_dynastycoin.coverage/index.html`.
```
...
To profile a running dynastycoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep dynastycoind` -- sleep 60
```
...
DynastyCoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as DynastyCoin Core, where everyone uses
...
pay attention to for reviewers of DynastyCoin Core code.

General DynastyCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: DynastyCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of DynastyCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against DynastyCoin Core so that they can be integrated
...
file descriptors, it will cause problems with DynastyCoin's `select()` loop, because
...
compatibility between nodes. This happened in DynastyCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to DynastyCoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dynastycoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dynastycoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of DynastyCoin Core, the wallet was always
