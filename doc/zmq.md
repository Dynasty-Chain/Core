...
The DynastyCoin Core daemon can be configured to act as a trusted "border
router", implementing the dynastycoin wire protocol and relay, making
...
The ZeroMQ feature in DynastyCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building dynastycoind:
...
For instance:
    ...
    $ dynastycoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/dynastycoind.tx.raw \
...
These options can also be provided in dynastycoin.conf.
...
From the perspective of dynastycoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into dynastycoind directly. Furthermore, no information is
...
using. DynastyCoind appends an up-counting sequence number to each
