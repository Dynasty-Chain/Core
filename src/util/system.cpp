...
const char * const BITCOIN_CONF_FILENAME = "dynastycoin.conf";
...
#else
    const char* pszModule = "dynastycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DynastyCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/DynastyCoin";
#else
    // Unix-like
    return pathRet / ".dynastycoin";
#endif
#endif
}
